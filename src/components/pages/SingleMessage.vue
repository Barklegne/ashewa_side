  ...
  <div>
    <div style="background-color:#F7F3F3;">
      <v-app-bar fixed dark style="font-weight:600;z-index:2000" color="#43DB80"
        ><v-icon @click="$router.go(-1)" left>mdi-arrow-left</v-icon></v-app-bar
      >
...
export default {
  ...
  created: function() {
    ...
    this.connection.onmessage = (event) => {
      ...
      if (data.message === true || data.message === false) {
        ...
        data.username !== this.user.username &&
        data.room_name !== "notification_room"
      ) {
        let chatFrame = {
  ...
  methods: {
    ...
    onMessageSubmit: function(message) {
      ...
      if (
        this.messages.some((message) => !message.content.includes(productLink))
      ) {
        ///
